...
export const Route = createFileRoute("/product/$id")({
  ...
  notFoundComponent: () => (
    <div className="container mx-auto px-4 py-20 text-center">
      ...
      <Link to="/shop" className="mt-4 inline-block text-primary hover:underline">← Retour à la boutique</Link>
    </div>
  ),
});
...
function ProductPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <button onClick={() => navigate({ to: "/shop" })} className="mb-6 inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-primary">
        <ChevronLeft className="h-4 w-4" /> Retour
      </button>

      <div className="grid gap-12 md:grid-cols-2">
        <div className="overflow-hidden rounded-3xl border bg-card">
          <img src={product.image} alt={product.title} width={800} height={800} className="aspect-square w-full object-cover" />
        </div>

        <div className="flex flex-col gap-4">
          <span className="text-sm font-medium uppercase tracking-wider text-secondary">{product.brand} · {product.category}</span>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-muted-foreground">{product.rating} · {product.reviewsCount} avis</span>
          </div>
          <div className="font-display text-4xl font-bold text-primary">{formatMAD(product.price)}</div>
          ...
          <div className="my-2 flex items-center gap-3">
            ...
            <Button
              ...
              onClick={() => {
                add({ productId: product.id, title: product.title, price: product.price, image: product.image }, qty);
          ...
          <div className="mt-2 grid grid-cols-2 gap-3 rounded-2xl bg-muted p-4 text-sm">
            <div className="flex items-center gap-2"><Truck className="h-4 w-4 text-primary" /> Livraison Maroc</div>
            <div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-primary" /> Garantie 1 an</div>
          </div>
        </div>
      </div>
      ...
      <section className="mt-16">
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-[1fr_400px]">
          <div className="space-y-4">
            {productReviews.length === 0 && <p className="text-muted-foreground">Aucun avis pour le moment. Soyez le premier !</p>}
              ...
              <div className="mt-4 space-y-3">
                <div>
                  ...
                  <div className="mt-1 flex gap-1">
                    {[1,2,3,4,5].map((n) => (
                      <button key={n} onClick={() => setRating(n)} type="button" className="text-2xl">
                        <span className={n <= rating ? "text-accent" : "text-muted-foreground/40"}>★</span>
                      </button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <Textarea id="comment" value={comment} onChange={(e) => setComment(e.target.value)} placeholder="Votre avis..." />
                </div>
                <Button
                  className="w-full"
                  onClick={() => {
                    if (!comment.trim()) return toast.error("Écrivez un commentaire");
                    addReview({ productId: id, author: user.name, rating, comment: comment.trim() });
                    setComment("");
